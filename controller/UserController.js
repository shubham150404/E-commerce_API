...
exports.User_create = async function (req, res, next) {
    try {
        ...
        //
        const roleCheck = ['customer', 'admin'];
        if (!roleCheck.includes(req.body.role)) {
            throw new Error("User role must be 'Customer','Admin'");
        }
        //
        ...
        const Jwt_User = jwt.sign({ id: User_data._id }, 'User')
        res.status(201).json({
            status: "sucess",
            message: "User create",
            data: User_data,
...
exports.User_sequre = async function (req, res, next) {
    try {
        ...
        const Jwt_token = jwt.verify(User_Token, 'User');
        next()
    } catch (error) {
        res.status(404).json({
            status: "faild",
